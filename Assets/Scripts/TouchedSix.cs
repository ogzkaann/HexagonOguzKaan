...
public class TouchedSix : MonoBehaviour
{
    ...
    public void OnMouseUp() // 2,3 noktası için 1,2 ve 2,2'yi seçmeli. Yani -1,-1 & 0,-1
    {
        ...
        if (transform.position.y > 0.4) // Alt Sınır Kontrolü
        {
            if (transform.position.x > 0) // Sol Sınır Konrolü
            {
                Destroy(GameObject.FindWithTag("Border1"));

                //Debug.Log("click 6");
                GameObject myBorder = GameObject.Instantiate(border, new Vector3(transform.position.x - 0.308f, transform.position.y - 0.338051f, -0.13f), transform.rotation * Quaternion.Euler(0f, 180f, 0f));
                ...
                ThirdHexCoordinatex = FirstHexCoordinatex;
