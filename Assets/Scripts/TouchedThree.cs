...
public class TouchedThree : MonoBehaviour
{
    ...
    public void OnMouseUp() // 2,3 noktası için 2,4 ve 3,3'ü seçmeli. Yani 0,+1 & +1,0
    {
        ...
        if (transform.position.y < 5.62) // Üst sınır kontrolü
        {
            if(transform.position.x < 4.26) // Sağ sınır kontrolü
            {
                ...
                //Debug.Log("click 3");
                GameObject myBorder = GameObject.Instantiate(border, new Vector3(transform.position.x + 0.301f, transform.position.y + 0.368719f, -0.13f), Quaternion.identity);
